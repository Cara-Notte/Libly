class LiblyNavigation {
    ...
    updateNavigationState() {
        ...
        if (this.isAuthenticated()) {
            ...
            // Add Books and Reviews links for all authenticated users
            ...
            navMenu.insertBefore(booksLink, navMenu.firstChild.nextSibling); // insert after Home but before others
            ...
            navMenu.insertBefore(reviewsLink, navMenu.firstChild.nextSibling.nextSibling); // insert after Books
            
            const loansLink = document.createElement('a');
            loansLink.href = 'loans.html';
            loansLink.className = 'nav-item';
            loansLink.textContent = 'Loans';
            navMenu.insertBefore(loansLink, navMenu.firstChild.nextSibling.nextSibling.nextSibling); // insert after Reviews
            ...
            if (this.currentUserRole === 'admin') {
                ...
                navMenu.insertBefore(adminLink, navMenu.firstChild.nextSibling.nextSibling.nextSibling.nextSibling); // insert after Loans
            }
            
            // If user is librarian, add a Librarian Panel link
            if (this.currentUserRole === 'librarian') {
                // Remove existing librarian link if present to avoid duplicates
                ...
                navMenu.insertBefore(librarianLink, navMenu.firstChild.nextSibling.nextSibling.nextSibling.nextSibling); // insert after Loans
            }
        } else {
            this.addAuthButtons(navMenu);
            // Add Books and Reviews links for non-authenticated users
            const existingBooks = navMenu.querySelector('a[href="books.html"]');
            if (existingBooks) existingBooks.remove();
            const booksLink = document.createElement('a');
            booksLink.href = 'books.html';
            ...
            // Remove librarian link if it exists when logging out
            const existingLibrarian = navMenu.querySelector('a[href="librarian.html"]');
            if (existingLibrarian) existingLibrarian.remove();
        }
    }
