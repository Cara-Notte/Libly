...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('reviewSortBy').addEventListener('change', loadAdminReviews);
...
async function loadAdminReviews() {
    try {
        ...
        if (response.ok) {
            ...
            updateAdminReviewsCount(data.pagination);
        } else {
            showAdminReviewsError('Failed to load reviews');
        }
    } catch (error) {
        console.error('Error loading admin reviews:', error);
        showAdminReviewsError('Network error. Please try again.');
    }
}

// Render reviews in admin panel
function renderAdminReviews(reviews) {
    ...
    reviews.forEach(review => {
        const reviewElement = createAdminReviewElement(review);
        container.appendChild(reviewElement);
    });
}

// Create admin review element
function createAdminReviewElement(review) {
...
// Update admin reviews count
function updateAdminReviewsCount(pagination) {
    const count = pagination.total;
    document.getElementById('adminReviewsCount').textContent = `Showing ${count} review${count !== 1 ? 's' : ''}`;
}
...
async function deleteReview(reviewId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            loadAdminReviews(); // Refresh the list
        } else {
...
// Show admin reviews error
function showAdminReviewsError(message) {
    const container = document.getElementById('adminReviewsList');
    container.innerHTML = `
